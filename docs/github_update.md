...
```json
{
  ...
  "current_branch": "main",
  "has_changes": false,
  "remote_url": "https://github.com/wemark-rss/wemark-rss.git",
...
```bash
# 每天凌晨 2 点自动检查更新
0 2 * * * cd /path/to/wemark-rss && python github_update.py --status --json > /var/log/github_status.log

# 每周日凌晨 3 点自动更新（带备份）
0 3 * * 0 cd /path/to/wemark-rss && python github_update.py --update --backup >> /var/log/github_update.log 2>&1
```
