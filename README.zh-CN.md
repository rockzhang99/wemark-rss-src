...
快速运行
```
docker run -d  --name wemark-rss  -p 8001:8001 -v ./data:/app/data  ghcr.io/wemark-rss/wemark-rss:latest
```
http://<您的ip>:8001/  即可开启
...
```
docker stop wemark-rss
docker rm wemark-rss
docker pull ghcr.io/wemark-rss/wemark-rss:latest
# 如果添加了其它参数，请自行修改
docker run -d  --name wemark-rss  -p 8001:8001 -v ./data:/app/data  ghcr.io/wemark-rss/wemark-rss:latest
```
...
# 官方镜像
```
docker run -d  --name wemark-rss  -p 8001:8001 -v ./data:/app/data  wemark-rss/wemark-rss:latest
```
# 代理镜像加速访问（国内访问速度更快）
```
docker run -d  --name wemark-rss  -p 8001:8001 -v ./data:/app/data  docker.1ms.run/wemark-rss/wemark-rss:latest  
```
...
# ❤️ 赞助
如果觉得 WeMark-RSS 对你有帮助，欢迎给我来一杯啤酒！<br/>
...
更多项目原理，请参考[项目文档](https://deepwiki.com/wemark-rss/wemark-rss/3.5-notification-system)。
...
[Star History Chart]: https://api.star-history.com/svg?repos=wemark-rss/wemark-rss&type=Timeline